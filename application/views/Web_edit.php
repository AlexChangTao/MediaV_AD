...
<div class="ibox-content main_content_margin float-e-margins">
    <form method="post" action="<?php echo site_url('C_Website/web_edit_page/').$web['id'].'/'.$adv_id; ?>" class="form-horizontal">
        <div class="form-group">
          ...
          <div class="col-lg-10">
            <input type="text" name="start" placeholder="选择时间" value="<?php echo $web['start_date']; ?>" class="form-control form_datetime">
          </div>
        </div>
        <div class="form-group">
          <label class="col-lg-2 control-label">结束时间</label>
          <div class="col-lg-10">
            <input type="text" name="end" placeholder="选择时间" value="<?php echo $web['end_date']; ?>" class="form-control form_datetime">
          </div>
        </div>
        <div class="form-group">
          <label class="col-lg-2 control-label">站点名称</label>
          <div class="col-lg-10">
            <input type="text" name="name" placeholder="站点名称" value="<?php echo $web['web_name']; ?>" class="form-control">
          </div>
        </div>
        <div class="form-group">
          <label class="col-lg-2 control-label">站点编码</label>
          <div class="col-lg-10">
            <input type="text" name="code" placeholder="站点编码" value="<?php echo $web['web_code']; ?>" class="form-control">
          </div>
        </div>
        <div class="form-group">
          <label class="col-lg-2 control-label">站点链接</label>
          <div class="col-lg-10">
            <input type="text" name="url"  placeholder="站点链接" value="<?php echo $web['web_url']; ?>" class="form-control">
          </div>
        </div>
        <div class="form-group">
          <label class="col-lg-2 control-label">站点说明</label>
          <div class="col-lg-10">
            <input type="text" name="remark" placeholder="站点说明" value="<?php echo $web['remark']; ?>" class="form-control">
          </div>
        </div>
        <div class="form-group">
          ...
          <div class="col-sm-10" >
           ...
           <!--  <button type="button" class="btn btn-success" id="show-values">确认</button>
            <span>选择之后请确认</span><br> -->
            <input type="hidden" readonly="true" value="<?php echo $web_area; ?>" name="area" id="values">
          </div>
        </div>
        <div class="form-group">
          ...
          <?php foreach ($media as $key => $value):?>
              <label class="checkbox-inline">
                  <input type="checkbox" name="media[]" value="<?php echo $value['id'] ?>" <?=in_array($value['id'], $web_media)?'checked':''; ?>><?php echo $value['media_name'] ?>
              </label>
          <?php endforeach; ?>
          </div>
        </div>
        <div class="form-group">
          ...
          <?php foreach ($models as $key => $value):?>
              <div class="">
                  <label class="checkbox-inline">
                      <input type="checkbox" name="feat[]" value="<?php echo $value['id'] ?>" <?=in_array($value['id'], $wm)?'checked disabled="disabled"':''; ?>><?php echo $value['name'] ?>
                  </label>
              </div>
          <?php endforeach; ?>
          </div>
